...
FROM "order" O
    LEFT JOIN "order_tax_lines" OTL ON O."id" = OTL."order_id"
GROUP BY 1,2,3,4;
...
FROM "order" O
        LEFT JOIN "order_fulfillments" OFU ON O."id" = OFU."order_id";
...
SELECT O."id"                                                        AS ORDER_ID
     ...
     , CASE WHEN LI."price" = '' AND LITL."price" = '' THEN 0 
...
FROM "line_item" LI
    ...
    LEFT JOIN "order_discount_applications" ODA ON O."id" = ODA."order_id"
    LEFT JOIN "line_item_tax_lines" LITL ON LI."id" = LITL."line_item_id"
    LEFT JOIN "product_variant" PV ON LI."variant_id" = PV."id"
    LEFT JOIN "inventory_items" II ON PV."inventory_item_id" = II."id";

--- Get billing and shipping details

CREATE OR REPLACE TABLE "bdm_shipping_type" AS
SELECT row_number() over(ORDER BY NAME) as SHIPPING_TYPE_ID,NAME FROM(
SELECT DISTINCT split_part(split_part("shipping_lines", '\'code\': \'',  2), '\', \'delivery_category',1) as NAME
FROM "order")t;

CREATE OR REPLACE TABLE "bdm_billing_type" AS 
SELECT ROW_NUMBER() OVER (ORDER BY NAME) as BILLING_TYPE_ID, NAME FROM (
SELECT ''/*DISTINCT concat('Method: ', "processing_method", ', Gateway: ',"gateway", 
                       CASE WHEN "payment_details__credit_card_company" <> '' 
...
SELECT DISTINCT 
 CASE 
    WHEN
    "payment_details__credit_card_company" IN ('Mastercard', 'Visa') AND "processing_method" = 'direct' AND "gateway" = 'stripe' THEN concat('On-line platba kartou - ', "payment_details__credit_card_company")
    WHEN
    "payment_details__credit_card_company" IN ('Mastercard', 'Visa') AND "processing_method" = 'manual' AND "gateway" = 'Bank Deposit' THEN concat('Dobírka - platba kartou (',"payment_details__credit_card_company",')')
    WHEN
    "payment_details__credit_card_company" = '' AND "processing_method" = 'manual' AND "gateway" = 'Bank Deposit' THEN 'Dobírka - hotově'
    WHEN
    "payment_details__credit_card_company" = '' AND "processing_method" = 'manual' AND "gateway" = 'stripe' THEN 'On-line bankovní převod'
    ELSE '' END as NAME
FROM "order") as t;
*/
